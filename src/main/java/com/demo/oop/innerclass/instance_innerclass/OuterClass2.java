...
public class OuterClass2 {
    ...
    class InnerClass{
        ...
        public void showName(){
            ...
            System.out.println(new OuterClass2().name);
        }
    }
    ...
    /**
     * 外部类实例方法中创建内部类对象，并通过内部类对象访问其方法
     *把实例成员内部类构造方法想象成外普类的一个普通方法 ，InnerClass就是这个方法的返回值
     */
    @Test
    public void test(){
        //这里实际上是OuterClass2.InnerClass in=this.new InnerClass(),外部类内部声明内部类对象时候会自动添加上外部类限制符
    ...
    public static void main(String[] args) {
        ...
        //外部类实例方法中创建内部类对象，并通过内部类对象访问其方法
        //实例成员内部类对象是依附外部类对象而存在的，也就是说，如果要创建实例成员内部类的对象，前提是必须存在一个外部类的对象
