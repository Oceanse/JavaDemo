 ...
 *  只不过对于父类私有属性，子类只能间接拥有(通过继承父类暴露的getter/setter/constructor等方法间接访问和修改)
 *  对于父类的私有方法，子类只能间接拥有，通过继承父类暴露的方法间接调用
 ...
 *
 *  Note:
 * 继承可以使用 extends 和 implements 这两个关键字来实现继承当一个类没有继承的两个关键字，则默认继承object,因此可以调用它的toString、equals等方法
