 ...
 * 标识符通常是指普通标识符，java中所有自己命名的位置所用的都是普通标识符: 接口名 类名 方法名 变量名 参数名
 ...
 * 变量三要素：数据类型 变量名 变量值
 *           变量类型决定内存空间大小；
 *           变量名就是这块内存空间的名字(同一个作用域内，变量名不能相同)；驼峰命名法(第一个单词以小写字母开始；从第二个单词开始以后的每个单词的首字母都采用大写字母)；变量名不能是关键字
 *           变量值就是变量内存空间存储的内容；Java语言是强类型语言，特定类型的变量只能存储特定类型的变量值
 *
 * 变量赋值：把数据存入指定的内存空间，基本类型变量存储字面值( A literal is the source code representation of a fixed value  without requiring computation，包括基本类型数值以及字符串)，引用类型变量存储对象的引用(地址)；
 *         变量赋值严格来说是把特定类型的变量值/数据存储到特定类型的变量空间中， 这里变量和变量值都具备类型属性，特定类型的变量只能接受特定类型的变量值
 ...
 *        关于null：null可以堪称是一种特殊的引用类型变量值，不能赋值给基本类型变量，可以赋值给任意引用类型的变量(null可以属于任何引用类型数据)，可以理解为内存空间存储着null这种不指向任何位置的特殊数据
 *   按作用域：局部变量 全局变量
 *
 * 变量类型转换：
 *   本质是对变量空间存储的变量值/数据进行类型转换，变量类型(接收的数据类型)并没有发生改变
...
public class VariableDemo {
    ...
    public void test() {
        ...
        //declare a variable of reference type and assign it the value which is the "address" of the object :
        ...
        //把变量flag存储的值赋值一份，然后取反再赋值给变量flag2，因此flag的取值并没有变
        boolean flag=true;
        boolean flag2=!flag;
        System.out.println(flag);
        System.out.println(flag2);

        //把变量存储的值复制一份然后和0.5计算后的结果赋值给变量total
        double total=score+0.5;
    ...
    /**
     * 变量类型转换：本质是对变量空间存储的变量值复制一份然后进行类型转换，变量本身(变量类型和变量值)并没有发生改变
     */
    @Test
    public void test2_1() {
        ...
        System.out.println(old);//old变量类型和存储的变量值都没有变
    ...
    public void test2_2() {
        ...
        System.out.println("1==========================");
        long l = 1;//1首先会自动类型转换为long类型，然后赋值给l
