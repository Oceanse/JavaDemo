...
public class ForDemo {
     ...
     */
    @Test
    public void test1_3() throws InterruptedException {
        for (int i = 1; i < 10; ) {
            System.out.println(i);
            Thread.sleep(1000);
            i++;
     ...
     */
    @Test
    public void test1_4() throws InterruptedException {
        for (int i = 1; i < 10; i++ ) {
            i*=0.1;
            Thread.sleep(1000);
            System.out.println(i);
    ...
    @Test
    public void test3() {
        int sum = 0;//这里可以把sum理解成个篮子,把所有符合条件的数据都放到这个篮子里
    ...
    public void test3_4() {
        ...
        int init = 2;//这里可以把result理解成个篮子，这里如果把init和m变量化(比如通过System.in控制台输入)，那么就可以求一个数的幂次方
        int m=16;
        int result=1;
        for (int i=1; i <=m;i++ ) {
            result *= init;
