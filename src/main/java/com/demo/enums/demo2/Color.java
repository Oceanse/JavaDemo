...
public enum Color {
    ...
    //@Test
    //这里不能实例化枚举类对象，所以不能执行单元测试，An error occurred while instantiating class com.demo.enums.demo2.Color. Check to make sure it can be instantiated
    public static void test(){
        //枚举类名.常量名
        Color red = Color.RED;//Color.RED相当于Person p中的p, 只是把这个常量对象的引用又赋值给了另一个引用，所以Color.RED和red都相当于常量对象的引用
    ...
    public static void test2(){
        //values()方法的作用就是获取枚举类中的所有实例对象，并作为数组返回
        Color[] values = Color.values();
        System.out.println(Arrays.toString(values));

        //据String名称获取枚举常量
        Color green = Color.valueOf("GREEN");//Color.valueOf("GREEN")相当于Color.GREEN，是一个常量对象的引用
        Color green2 = Enum.valueOf(Color.class,"GREEN");
        System.out.println(green.name());
        System.out.println(green2.name());

        //No enum constant com.demo.enums.demo2.Color.BLACK
    ...
    /**
     * name()和toString()等价，返回枚举常量的字符串形式
     */
    ...
    public static void test4(){
        ...
        switch (color) {//switch支持枚举类
            case RED: //这里可不以Color.RED, An enum switch case label must be the unqualified name of an enumeration constant
                System.out.println("red");
                break;
            case BLUE:
                System.out.println("blue");
