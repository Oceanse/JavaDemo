 ...
 * Exception normally disrupts the normal flow of the application that is why we use com.demo.exception handling. Let's take a scenario:
 *
 * statement 1;
 * statement 2;
 * statement 3;  Exception occurs and disrupts the normal flow of the application
 * statement 4;
 * statement 5;
 * statement 6;
 *
...
public class Try_catch_Demo {
    ...
    public void test1_2() {
        int[] array = new int[5];
        try {
            int re = array[0] / 0;  //系统自动抛出一个内置异常对象给运行时系统，默认处理方式是打印该对象的信息

            //上面的异常对象不属于catch声明的异常类，该对象不能被捕获，还是以默认方式（打印该对象的信息,并中断程序)处理，相当于没有catch,所以catch后面的代码不能被执行
